...
async def create_restaurant(db: DbDependency, restaurant_create: RestaurantCreate): # type: ignore
...
async def login(db: DbDependency, form_data: FormDependency): # type: ignore
...
async def create_tag(db: DbDependency, tag_create: TagCreate, token:str = Depends(JWTBearer())): # type: ignore
...
async def get_dish(db:DbDependency, t_id: Optional[int] = Query(None, gt=0, examples=[1]), token:str = Depends(JWTBearer())): # type: ignore
    r_name, r_id = restaurant_auth_cruds.decode_token(token)
    if t_id:
...
async def create_dish(db: DbDependency, dish_create: DishCreate, token:str = Depends(JWTBearer())): # type: ignore
    r_name, r_id = restaurant_auth_cruds.decode_token(token)
...
async def update_dish(db: DbDependency, f_id: int, dish_update: DishUpdate, token:str = Depends(JWTBearer())): # type: ignore
    r_name, r_id = restaurant_auth_cruds.decode_token(token)
    if not r_id:
        raise HTTPException(status_code=401, detail="Invalid token")
