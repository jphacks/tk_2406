...
async def create_order(db: DbDependency, order_create: OrderCreate, r_id: int=Path(gt=0), c_id: int = Query(None, gt=0, examples=[1])):
    return order_cruds.create(db, order_create, r_id, c_id)